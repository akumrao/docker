...
OsMutex CcuKafkaMessage::mAlertDbMutex = OsMutex ();
map<string, AlertDb> CcuKafkaMessage::mAlertDb = map<string, AlertDb> ();
...
{
	struct timeval mediadocTime;
   	memset (&mediadocTime, 0, sizeof(mediadocTime));
   	gettimeofday(&mediadocTime, 0);
   	time_t mediadocTimeMs = (mediadocTime.tv_sec)*1000+(mediadocTime.tv_usec)/1000;

	string kfMsgKey = std::to_string(mediadocTimeMs);
...
CcuKafkaMessage::createTxIdKey ()
{
	...
	sprintf(bufP, "%ld-%ld", mediadocTime.tv_sec, mediadocTime.tv_usec);
	string txIdKey(bufP);
...
CcuKafkaMessage::sendLogMessage (Producer & kfProducer, const string & txIdKey, const string & appName, const string & subsystemName, const string & level, const string & textMessageStr, const string & jsonMessageStr)
{
	...
	//
	// "mediadoc-time": <ISO8601 Timestamp>,
	// "mediadoc-txIdKey":"<Tracking UUID>",
	// "mediadoc-source" : {
	//	"type":"LOCAL-APP"
	//	"address":"<Application Name>"
	// }
	// "mediadoc-recordType": "WHEELER-LOG-MSG",
	// "mediadoc-data" : {
	//	"log-message" : {
	...
	string kfPayload = "{\"mediadoc-time\":" + std::to_string(mediadocTimeMs) + ",";

	if (txIdKey.length() == 0) {
		kfPayload += "\"mediadoc-txIdKey\":\"" + CcuKafkaMessage::createTxIdKey() + "\",";
	} else {
		kfPayload += "\"mediadoc-txIdKey\":\"" + txIdKey + "\",";
	}
	kfPayload += "\"mediadoc-source\":{\"type\":\"LOCAL-APP\",\"address\":\"" + appName + "\"},";
	kfPayload += "\"mediadoc-recordType\":\"WHEELER-LOG-MSG\",\"mediadoc-data\":{\"log-message\":{";
...
CcuKafkaMessage::sendAlert (Producer & kfProducer, const string & txIdKey, const string & alertId, const string & modelName, bool isActive, const string & textMessageStr, const string & jsonMessageStr)
{
	...
	//	"address":"<Application Name>"
	// }
	// "mediadoc-recordType": "WHEELER-ALERT",
	// "mediadoc-data" : {
	//	"alert-message" : {
	...
	kfPayload += "\"mediadoc-source\":{\"type\":\"STATION-SERIAL-NUMBER\",\"address\":\"" + mqisSerialNumber + "\"},";
	kfPayload += "\"mediadoc-recordType\":\"WHEELER-ALERT\",\"mediadoc-data\":{\"alert-message\":{";
